...
| 4b | Permissions (documented UI) | Editor opens "Page options" (page-type management) — gated by `serverRole/siteRole === 'developer'` in `Toolbar.svelte:246` | Control is hidden/inaccessible in UI for editor | UI assertion (locator not visible / not present) |
