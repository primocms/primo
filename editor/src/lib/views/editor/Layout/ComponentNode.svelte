      ...
      if (res.error) {
        ...
        component = new App({
          ...
          props: componentData,
				...
				const elements_with_text = Array.from(node.querySelectorAll('*')).filter((element) => {
        ...
        async function set_editable({ element, key = '', should_create_editor = false }) {
          element.style.outline = '0';

          if (should_create_editor) {
            local_html = element.innerHTML
              .trim();
