...
<script>
  ...
  import {
    static_iframe_srcdoc,
    component_iframe_srcdoc,
  } from '$lib/builder/components/misc'
  import {
    design as siteDesign,
    code as siteCode,
  } from '$lib/builder/stores/data/site.js'
  ...
  async function set_editable_markdown({ id, key, element }) {
    ...
    const markdown_id =
      element.getAttribute('data-markdown-id') || createUniqueID()
  ...
  async function generate_component_code(raw_code) {
    ...
    if (res.error) {
      error = res.error
      console.log('generate_component_code error')
      dispatch_mount()
    } else {
      error = ''
  ...
  async function make_content_editable() {
    ...
    const valid_elements = Array.from(
      doc.querySelectorAll(`img, a, p, span, h1, h2, h3, h4, h5, h6, div`),
    ).filter((element) => {
    ...
    const static_fields = block.fields.filter((f) =>
      static_field_types.includes(f.type),
    )

    for (const field of static_fields) {
      const relevant_entries = section.entries.filter(
        (e) => e.field === field.id,
      )
  ...
  function updateMenuPositions() {
    ...
    if (selection?.rangeCount > 0) {
      ...
      const isTopLevelBlock =
        blockElement?.parentElement?.matches('.ProseMirror')
  ...
  async function mount_component(js) {
    ...
    try {
      ...
      console.log('mount_component error')
      error = e
      dispatch_mount()
    }
...
<div class="menu floating-menu primo-reset" bind:this={floating_menu}>
  {#if active_editor}
    ...
    <MarkdownButton
      icon="fa-solid:code"
      onclick={() => active_editor.chain().focus().toggleCodeBlock().run()}
    />
    <MarkdownButton
      icon="fa-solid:quote-left"
      onclick={() => active_editor.chain().focus().toggleBlockquote().run()}
    />
    <MarkdownButton
      icon="fa-solid:list"
      onclick={() => active_editor.chain().focus().toggleBulletList().run()}
    />
    <MarkdownButton
      icon="fa-solid:list-ol"
      onclick={() => active_editor.chain().focus().toggleOrderedList().run()}
    />
    <MarkdownButton
      icon="fa-solid:image"
      onclick={() =>
...
<div class="menu bubble-menu primo-reset" bind:this={bubble_menu}>
  {#if active_editor}
    ...
    <MarkdownButton
      icon="fa-solid:bold"
      onclick={() => active_editor.chain().focus().toggleBold().run()}
      active={active_editor.isActive('bold')}
    />
    <MarkdownButton
      icon="fa-solid:italic"
      onclick={() => active_editor.chain().focus().toggleItalic().run()}
      active={active_editor.isActive('italic')}
    />
    <MarkdownButton
      icon="fa-solid:highlighter"
      onclick={() => active_editor.chain().focus().toggleHighlight().run()}
