<script>
  ...
  import {
    code as siteCode,
    design as siteDesign,
  } from '$lib/builder/stores/data/site'
  import {
    locale,
    locked_blocks,
    page_loaded,
    dragging_symbol,
  } from '$lib/builder/stores/app/misc'
  import {
    add_section_to_palette,
    delete_section_from_palette,
    move_section,
  } from '$lib/builder/actions/sections'
  ...
  import {
    attachClosestEdge,
    extractClosestEdge,
  } from '$lib/builder/libraries/pragmatic-drag-and-drop-hitbox/closest-edge.js'
    ...
    dropTargetForElements({
      ...
      async onDrag({ self, source }) {
        ...
        const last_section_id =
          palette_sections[palette_sections.length - 1]?.id
        if (!last_section_id) return
        hovered_block_el = page_el.querySelector(
          `[data-section="${last_section_id}"]`,
        )
        ...
        if (
          dragging.id !== last_section_id ||
          dragging.position !== extractClosestEdge(self.data)
        ) {
          dragging = {
            id: last_section_id,
      ...
      async onDrop({ source }) {
        ...
        const new_section_el = page_el.querySelector(
          `[data-section="${new_section_id}"]`,
        )
  ...
  function drag_item(element, section) {
    dropTargetForElements({
      ...
      async onDrag({ self, source }) {
        ...
        position_drop_indicator()
        if (
          dragging.id !== self.data.section.id ||
          dragging.position !== extractClosestEdge(self.data)
        ) {
          dragging = {
      ...
      async onDrop({ self, source }) {
        ...
        const section_dragged_over_index = $sections.find(
          (s) => s.id === self.data.section.id,
        ).index
        const block_being_dragged = source.data.block
        ...
        if (closestEdgeOfTarget === 'top') {
          ...
          const new_section_id = await add_section_to_palette({
            palette_id: palette_section.id,
            symbol: block_being_dragged,
            position: is_first_pallete_section
              ? 0
              : section_dragged_over_index + 1,
          })
          const new_section_el = page_el.querySelector(
            `[data-section="${new_section_id}"]`,
          )
  ...
  let non_palette_sections = $derived(
    $sections.filter((s) => s.palette || s.master.symbol),
  )
  $effect(() => {
    if (
      sections_mounted === non_palette_sections.length &&
      sections_mounted !== 0
    ) {
      page_mounted = true
  ...
  let palette_section = $derived(
    $sections.find((s) => !s.symbol && !s.master?.symbol),
  )
  let block_toolbar_on_locked_block = $derived(
    $locked_blocks.find((b) => b.block_id === hovered_section?.id),
  )
  $effect(() => {
...
<main
  id="Page"
  bind:this={page_el}
  class:fadein={$page_loaded && page_mounted}
  class:dragging={$dragging_symbol}
  lang={$locale}
  use:drag_fallback
>
  {#each top_level_sections.sort((a, b) => a.master.index - b.master.index) as section (section.id)}
    ...
    {@const show_block_toolbar_on_hover =
      !moving && !(is_palette && palette_sections.length === 0)}
    {@const has_page_type_symbols = $symbols.some((s) =>
      s.page_types.includes($active_page.page_type.id),
    )}
    {#if is_palette && has_page_type_symbols}
      <div
        data-section={section.id}
        data-type="palette"
        class:empty={palette_sections.length === 0}
      >
        {#if palette_sections.length > 0}
          {#each palette_sections.sort((a, b) => a.index - b.index) as section (section.id)}
            ...
            {@const locked = $locked_blocks.find(
              (b) => b.block_id === section.id,
            )}
    ...
    {:else if !is_palette}
      {@const block = $symbols.find(
        (block) => block.id === section.master.symbol,
      )}
