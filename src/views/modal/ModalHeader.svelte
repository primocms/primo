<script lang="ts">
  ...
  export let button:{ label:string, icon:string, onclick: any } = null
</script>

<header class="modal-card-head {variants}">
  <div class="flex-1 flex justify-start">
    <button id="modal-close" on:click={() => dispatch('close')} type="button" class="inline-flex items-center justify-center rounded-md text-gray-400 p-2 hover:text-gray-500 hover:bg-gray-200 focus:outline-none focus:bg-gray-200 focus:text-gray-500 transition duration-150 ease-in-out" aria-label="Close modal">
  ...
  <div class="flex-1 flex justify-end">
    ...
    {/if}
    {#if button}
      <button class="button primary" on:click={button.onclick}>
        {#if button.icon}
