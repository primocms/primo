...
const sveltePromiseWorker = new PromiseWorker(SvelteWorker);

// Clear out storage to avoid mucking it up
// idb.clear()

export async function svelte({ code, data, buildStatic = true, format = 'esm'}) {
  ...
  let res
  try {
    res = await sveltePromiseWorker.postMessage(finalRequest)
  } catch(e) {
    console.log('error', e)
    res = {
      error: e.toString()
