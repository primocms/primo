<script>
  ...
  import { compileScss, processStyles, wrapInStyleTags, buildPageHTML, buildSiteHTML, getComponentCSS } from 'utils'

  import {content,domainInfo,settings,site,tailwind,getCombinedTailwindConfig} from '@stores/data'
  import {modal} from '@stores/app'

  let { compiled:pageStyles } = $settings.globalStyles
  let { styles } = $site
  ...
  onMount(async() => {
    ...
    siteHTML = await buildSiteHTML($domainInfo.page, JSON.stringify(combinedTailwindConfig))
    allPages = [ pageHTML, ...siteHTML ]
  })
  ...
  let moduleTailwindConfig = styles.tailwind;
