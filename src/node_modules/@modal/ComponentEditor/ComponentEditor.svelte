<script lang="ts">
  ...
  import {getSymbol,saveSymbolToDomain,updateInstancesInDomain} from '@fb/firestore/domains'
  ...
  import {settings, dependencies, content, site, user} from '@stores/data'
  ...
  async function compileHtml(html:string): Promise<void> {
    ...
    if (fields.length > 0 || rawHTML.includes('nav')) { 
      const fieldsWithNav = includeNavField(fields)
      const data = await convertFieldsToData(fieldsWithNav, 'all')
  ...
  async function updateHtmlWithFieldData(typeToUpdate:string): Promise<void> {
    loading = true
    const fieldsWithNav:Fields = includeNavField(fields)
    let data = await convertFieldsToData(fieldsWithNav, typeToUpdate)
  ...
  async function loadSymbol(): Promise<void> {
    disabled = false
    const symbol:Component = await getSymbol(localComponent.symbolID)
  ...
  // TODO: Get user roles working again
  const tabs = [
    {
      label: 'Content',
      icon: 'edit'
  ...
  // let activeTab = $user.role === 'developer' ? tabs[1] : tabs[0]
      ...
      <div class="flex flex-col h-full">
        ...
        {#if activeTab === tabs[0]}
          {#each fields as field}
              ...
              <Card title={field.label} variants="p-2 pb-4">
                {#each field.value as item (item.id)} 
                  ...
                  >
                    ...
                    <div slot="image">
                      <ImageField 
                        imageUrl={item[subfield.key]}  
                        on:submit={({detail}) => {
                          item[subfield.key] = detail.url;
                          updateHtmlWithFieldData('static')
                        }} 
                      />
                    </div>
            ...
            {/if}
          {:else}
            <h2 class="title is-6 mt-4">Add a field to easily change content here</h2>
          {/each}
        ...
        {:else if activeTab === tabs[1]}
          ...
          <div class="flex flex-col mt-2">
            ...
            {/each}
            <button class="field-button" on:click={addField} {disabled}>Add a Field</button>
          </div>
        {/if}
      </div>
    </div>
...
<style>
  .field-button {
    @apply w-auto bg-gray-800 text-gray-300 py-2 rounded font-medium transition-colors duration-200;
    &:hover {
      @apply bg-gray-900;
    }
    &[disabled] {
