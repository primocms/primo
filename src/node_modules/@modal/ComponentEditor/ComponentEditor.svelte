<script lang="ts">
  ...
  async function compileHtml(html:string): Promise<void> {
    ...
    const fieldsWithSiteData = [ ...$site.fields, ...fields]
    const data = await convertFieldsToData(fieldsWithSiteData, 'all')
  ...
  async function updateHtmlWithFieldData(typeToUpdate:string): Promise<void> {
    loading = true
    const fieldsWithSiteData:Fields = [ ...$site.fields, ...fields ]
    let data = await convertFieldsToData(fieldsWithSiteData, typeToUpdate)
