...
import {Theme,Font,Preferences} from 'constructs'
import {setPagePassword,getAllPages} from '@fb/firestore/domains'
import ShortUniqueId from "short-unique-id";
import objectPath from 'object-path'

import {content,settings,dependencies,site,getCombinedTailwindConfig,domainInfo,user} from '@stores/data'
...
export async function parseHandlebars(code, data) {
    ...
    if (window.Handlebars) {
      const template = window.Handlebars.compile(code);
      html = template(data)
    } else {
      html = await ax.post('primo/handlebars', {
        source : code,
        data
      })
    }
...
export async function getEmptyData(identity = { title: '', url: '' }) {
  return { 
    ...
    settings: {
      ...
      globalStyles: await getDefaultPageStyles()
    },
    dependencies: {
      libraries: [],
      headEmbed: ''
...
export async function processStyles(css, html, options = {}) {
  const {data:result} = await axios.post( get(domainInfo).onDev ? 'http://localhost:3000/postcss' : 'https://primo-functions.herokuapp.com/postcss', { css, html, options })
  if (result.error) {
...
const boilerplate = async (html, { customScripts, identity }, { libraries, headEmbed }) => {
  return `
    <!doctype html>
    ...
    <head>
      ...
      <link rel="stylesheet" type="text/css" href="./${get(domainInfo).page}.css" />
      <script src="./${get(domainInfo).page}.js"></script>
      ${
        libraries.length > 0 
      ...
      // ${libraries.map(l => 
      //   l.type === 'js'
      //   ? `<script src="${l.src}"></script>`
      //   : `<link rel="stylesheet" href="${l.src}" />` 
      // ).join('\n')}
      // `${customScripts.map(s => `<script src="${s.src}"></script>`).join('\n')}
      `${headEmbed}
    </head>
...
export async function buildSiteHTML(exclude = null) {
  const pages = await getAllPages(exclude);
  const pagesHTML = await Promise.all(await pages.map(async page => {
    const HTML = await buildPageHTML(page.content)
    
    const tailwindConfig = JSON.stringify(getCombinedTailwindConfig(page.settings.tailwindConfig))
    const CSS = await buildPageCSS(page.content, HTML, page.settings.globalStyles.uncompiled, tailwindConfig)
...
export async function buildPageHTML(content, buildWholePage = false) {
  ...
  return buildWholePage ? await boilerplate(html, get(settings), get(dependencies)) : html
}

export async function buildPagePreview(page) {
  let html = ''
  const { content, settings } = page
  content.forEach(section => {
    html += `<section id="section-${section.id}">\n` +
              `\t<div class="columns flex flex-wrap ${section.width === 'contained' ? 'container' : ''}">\n`
    section.columns.forEach(column => {
  ...
  html += settings.globalStyles.compiled
...
export async function buildPageFiles(content, { javascript, globalStyles }) {
  ...
  const combinedCSS = get(site).styles.raw + globalStyles.uncompiled
