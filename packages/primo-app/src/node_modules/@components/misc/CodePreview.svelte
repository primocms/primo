...
<div class="h-full flex flex-col pl-2">
  <div class="preview-container flex-1" class:loading bind:this={container}>
    <iframe class:scaled={view === 'large'} on:load={() => {iframeLoaded = true}} class:fadein={iframeLoaded} title="Preview HTML" src="/?preview={multiple ? 'multiple' : true}" class="bg-white w-full h-full" bind:this={iframe}></iframe>
  </div>
  <div class="footer-buttons">
    ...
    {/if}
    <a target="blank" class="separate-tab" href="/?preview={multiple ? 'multiple' : true}">
