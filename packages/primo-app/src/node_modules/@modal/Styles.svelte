<script>
  ...
  function buildPreview(css, content) {
    return {
      html: wrapInStyleTags(siteStyles.final)
        + wrapInStyleTags(css) 
        + buildPagePreview(content)
    }
  }

  $: currentPage = buildPreview(pageStyles.final, $content) 
  $: allPages = $site.pages.map(page => page.id === $pageData.id ? currentPage : buildPreview(page.styles.final, page.content))
